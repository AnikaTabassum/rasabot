...
function botInitiates(message) {
	message="hi";
	user_id = "LIC";

	$.ajax({
		url: "http://localhost:5005/webhooks/rest/webhook",
		// url: "http://115.127.24.183:5005/webhooks/rest/webhook",
		// url: "http://192.168.20.62:5005/webhooks/rest/webhook",
		type: "POST",
...
// 	$.ajax({
// 		url: `http://2f60195d.ngrok.io/conversations/${user_id}/execute`,
// 		//url: `https://2f60195d.ngrok.io/webhook/rest/webhook`,
// 		type: "POST",
// 		contentType: "application/json",
		...
		if (response.length < 1) {
			...
			for (i = 0; i < response.length; i++) {
				...
				if (response[i].hasOwnProperty("text")) {
					...
					strArr=response[i].text.split("<br/>");
					// printing each line in a seperate message
					for(j=0;j<strArr.length;j++){
						var BotResponse = '<img class="botAvatar" src="./static/img/botAvatar.png"/><p class="botMsg">' + strArr[j]+ '</p><div class="clearfix"></div>';
						$(BotResponse).appendTo(".chats").hide().fadeIn((j+1)*400);
				}
				}
				...
				if (response[i].hasOwnProperty("buttons")) {
					...
					handleButton(response[i].buttons);
				}
...
function handleButton(textToAdd){
	setTimeout(function () {
		...
		var len=suggestions[i].title.length;
		if (len>=35){
		flag=25;
		}
		...
		var t=suggestions[i].title.replace("'","/");
		console.log("25");
		$('<button type="button" class="button2" onclick="payloading(\'' + suggestions[i].payload+",/"+t+ '\')">' + suggestions[i].title+ "</button>").appendTo(".menu");
		}
		}
		else{
		for (i = 0; i < suggLength; i++) {
		...
		var t=suggestions[i].title.replace("'","/");
		console.log("925");
		$('<button type="button" class="button" onclick="payloading(\'' + suggestions[i].payload+",/"+t+ '\')">' + suggestions[i].title+ "</button>").appendTo(".menu");
		}
		// $('<button class="btn btn-default" onclick="payloading(\'' + suggestions[i].payload+",/"+t+ '\')">' + suggestions[i].title+ "</button>").appendTo(".menu");
		// $('<button type="button" className="button" onClick="invoke"  name='payloading' data-arg1=suggestions[i].payload '> suggestions[i].title'</button>').appendTo(".menu");
		}
		scrollToBottomOfResults();
		}, 500);
		}
...
function addSuggestion(textToAdd) {
	setTimeout(function () {
		...
		for (i = 0; i < suggLength; i++) {
			...
			// }
			// else
			// $('<div class="singleButton" data-payload=\'' + (suggestions[i].payload) + '\'>' + suggestions[i].title+ "</div>").appendTo(".menu");
		}
		scrollToBottomOfResults();
	}, 1000);
}
...
function payloading(palod){
	console.log("anika", palod);
	var arr=palod.split(",/")
	var text = arr[1].replace("/","'");
	// var text=palod.title;
	var payLoadForButton=arr[0];
	// var payLoadForButton=palod.payload;
	console.log("inner text ",text);
	
	setUserResponse(text);
	send(payLoadForButton);
}
